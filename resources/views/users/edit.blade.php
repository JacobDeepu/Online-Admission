            ...
            <div class="relative overflow-hidden bg-white shadow-xl dark:bg-gray-800 sm:rounded-lg">
                ...
                <form class="p-4" method="POST" action="{{ route('users.update', $user) }}">
                    ...
                    <div>
                        ...
                        <x-input class="mt-1 block w-full" id="name" name="name" type="text" :value="old('name', $user->name)" required autofocus />
                    </div>
                    <div class="mt-4">
                        <x-label for="email" value="{{ __('Email') }}" />
                        <x-input class="mt-1 block w-full" id="email" name="email" type="email" :value="old('email', $user->email)" required autofocus />
                    </div>
                    <div class="mt-4">
                        <x-label for="password" value="{{ __('Password') }}" />
                        <x-input class="mt-1 block w-full" id="password" name="password" type="password" required autofocus />
                    </div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        @forelse ($roles as $role)
                            <label class="inline-flex" for="roles">
                                <x-checkbox name="roles[]" value="{{ $role->name }}" :checked="in_array($role->id, $userHasRoles)" />
