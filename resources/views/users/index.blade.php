        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto bg-white shadow-xl dark:bg-gray-800 sm:rounded-lg">
                <div class="flex-column flex flex-wrap items-center justify-between space-y-4 p-4 sm:flex-row sm:space-y-0">
                    ...
                    <div>
                        @can('create a user')
                            <x-link class="bg-indigo-700 text-white hover:bg-indigo-800 focus:ring-indigo-300" href="{{ route('users.create') }}">
                    ...
                    <tbody>
                        @forelse ($users as $user)
                            <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
                                <th class="whitespace-nowrap px-6 py-4 font-medium text-gray-900 dark:text-white" scope="row">{{ $user->name }}</th>
                                <td class="px-6 py-4">{{ $user->email }}</td>
                                @can('edit a user')
                                    <td class="flex items-center px-6 py-4">
                                        <x-link class="bg-indigo-700 text-white hover:bg-indigo-800 focus:ring-indigo-300" href="{{ route('users.edit', $user) }}">
                                    ...
                                    @endcan
                                    @can('delete a user')
                                        <form class="inline-block px-2" method="POST" action="{{ route('users.destroy', $user) }}">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
                                <td class="whitespace-nowrap px-6 py-4 font-medium capitalize text-gray-900 dark:text-white" scope="row">
                                    {{ __('No Users Found') }}
                                </td>
                            </tr>
                        @endforelse
