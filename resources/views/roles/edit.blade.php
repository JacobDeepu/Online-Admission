    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="relative overflow-hidden bg-white shadow-xl dark:bg-gray-800 sm:rounded-lg">
                ...
                <form class="p-4" method="POST" action="{{ route('roles.update', $role) }}">
                    @csrf
                    @method('PUT')
                    <div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        @forelse ($permissions as $permission)
                            <label class="inline-flex" for="perrmission">
                                <x-checkbox name="permissions[]" value="{{ $permission->name }}" :checked="in_array($permission->id, $roleHasPermissions)" />
